pub mod cli {
    ...
    fn get_content_loaders(args: std::slice::Iter<String>) -> HashMap<String, Box<dyn search::ContentLoader>> {
        let mut content_loaders: HashMap<String, Box<dyn search::ContentLoader>> = HashMap::new();
    ...
    pub fn process_command(args: Vec<String>) -> u32 {
        ...
        for direntry in WalkDir::new(".") {
            ...
            let mut commands: Vec::<search::SuperContentScorer> = Vec::new();
                ...
                if arg == "--more" {
                    match iter.next() {
                        Some(next) => {
                            ...
                            let b = Box::new(search::More{target: next.clone()});
                            commands.push(search::SuperContentScorer::new(b, String::from(content)));
                        },
                        None => { panic!("Expected a term after more"); }
                    }
                }
